...
function Logo() {
  return (
    <div className="flex items-center gap-2 select-none">
      <span className="text-xl" style={{ color: gold }}>⚖️</span>
...
function SectionHeading({ id, number, title }: { id: string; number: string; title: string }) {
  return (
    <h2 id={id} className="text-xl font-bold mt-12 mb-4 pt-4 scroll-mt-20 flex items-baseline gap-3" style={{ color: navy, fontFamily: 'var(--font-inter), system-ui, sans-serif' }}>
      <span className="text-sm font-bold px-2 py-0.5 rounded text-white" style={{ background: navy }}>{number}</span>
      {title}
    </h2>
  );
}
...
export default function StacyReport() {
  ...
  return (
    <div className="min-h-screen" style={{ background: bg, fontFamily: 'var(--font-inter), system-ui, sans-serif' }}>
      {/* ── Sticky Header ── */}
      <header className="sticky top-0 z-50 backdrop-blur-md border-b" style={{ background: 'rgba(248,249,251,0.92)', borderColor: '#e5e7eb' }}>
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <Logo />
          <div className="flex items-center gap-3">
            <span className="hidden sm:inline text-xs font-medium px-2.5 py-1 rounded-full" style={{ background: '#e0e7ff', color: navy }}>Research Report</span>
            {/* mobile TOC toggle */}
      ...
      <div className="max-w-5xl mx-auto px-4 flex gap-8 relative">
              ...
              {tocSections.map(s => (
                <li key={s.id}>
                  <button
                    ...
                    className={`block w-full text-left text-xs py-1.5 px-2.5 rounded transition-colors ${activeId === s.id ? 'font-semibold text-white' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-100'}`}
        ...
        <main ref={mainRef} className="flex-1 max-w-[800px] py-8 pb-20">
          {/* Title block */}
          <div className="mb-8">
            <h1 className="text-3xl sm:text-4xl font-bold leading-tight mb-3" style={{ color: navy }}>
              Removing a Housemate from<br className="hidden sm:block" /> Your Home in Topanga, CA
            </h1>
            <p className="text-sm text-gray-500">Research Guide — January 2026</p>
          </div>
          ...
          <p className="text-sm text-gray-600 mb-4">Evidence compiled from WhatsApp chat logs:</p>
          ...
          <div className="grid sm:grid-cols-3 gap-4 my-4">
            {[
              {
                title: '🚔 Criminal Route',
                items: ['Penal Code § 594 (Vandalism)', 'Under $400 = misdemeanor', '$400+ = misdemeanor or felony', 'File police report: (310) 456-6652'],
                bg: '#fef2f2', border: '#ef4444'
              },
              {
                title: '⚖️ Civil Route',
                items: ['Include in UD complaint', 'Or file small claims (up to $10k)', 'Filing fee: $30–$75'],
                bg: '#eff6ff', border: '#3b82f6'
              },
              {
                title: '🛡️ In Your RO',
                items: ['Evidence of threatening behavior', 'Request restitution / payment', 'Strengthens your case'],
          ...
          {/* ── Footer ── */}
          <footer className="mt-16 pt-8 border-t border-gray-200 text-center space-y-2">
            <p className="text-sm font-semibold" style={{ color: navy }}>
              Prepared by <span style={{ color: gold }}>JusticeOS.ai</span> — This is <strong>NOT</strong> legal advice
            </p>
            ...
            <p className="text-[10px] text-gray-300 mt-4">Powered by Empathy Labs</p>
          </footer>
        </main>
      </div>
      ...
      <style jsx global>{`
        @media print {
          header, aside, nav, button[aria-label="Table of contents"] { display: none !important; }
