...
const silverParticleVertex = `
  ...
  void main() {
    ...
    // Audio Reactivity: Bass expands the blast
    blast += uBass * 1.0 * smoothstep(0.0, 0.2, progress);
    ...
    // Audio Reactivity: Mid adds turbulence
    flowInfluence *= (1.0 + uMid * 2.0);
...
const CinematicIntro = ({ onScrollRequest, getAudioData }: { onScrollRequest: () => void, getAudioData: () => { bass: number, mid: number, high: number } }) => {
    ...
    useEffect(() => {
        ...
        const loop = () => {
            ...
            // Pass to Particles
            particleMaterial.uniforms.uBass.value = fieldMat.uniforms.uBass.value;
            particleMaterial.uniforms.uMid.value = fieldMat.uniforms.uVoice.value;
